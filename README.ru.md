...
### ⚠️ macOS — первый запуск (важно)
Приложение **не подписано платным сертификатом Apple** ($99/год), поэтому при первом запуске macOS может написать, что оно **«повреждено и не может быть открыто»** или **«от неизвестного разработчика»**. С приложением всё в порядке — это Gatekeeper осторожничает с неподписанными программами.

**Как открыть:**
1. Перетащи **WebP Squeeze** в **Applications** (Программы).
2. Правый клик по приложению → **Открыть** → **Открыть**.

Если всё равно пишет **«повреждено»** — открой **Терминал** и выполни один раз, потом запусти как обычно:
```bash
xattr -cr "/Applications/WebP Squeeze.app"
```
> В самом DMG также лежит файл **«READ ME FIRST»** с этой инструкцией на русском и английском.
